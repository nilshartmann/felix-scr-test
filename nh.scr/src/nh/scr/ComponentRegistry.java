...
/**
 * The <code>ComponentRegistry</code> class acts as the global registry for components by name and by component ID. As
 * such the component registry also registers itself as the {@link ScrService} to support access to the registered
 * components.
 */
public class ComponentRegistry implements ScrService {

  /**
   * The map of known components indexed by component name. The values are either the component names (for name
   * reservations) or implementations of the {@link ComponentHolder} interface.
   * <p>
   * The {@link #checkComponentName(String)} will first add an entry to this map being the name of the component to
   * reserve the name. After setting up the component, the {@link #registerComponent(String, ComponentHolder)} method
   * replaces the value of the named entry with the actual {@link ComponentHolder}.
   * 
  ...
  /**
   * Map of components by component ID. This map indexed by the component ID number (<code>java.lang.Long</code>)
   * contains the actual {@link AbstractComponentManager} instances existing in the system.
   * 
  ...
  /**
   * Counter to setup the component IDs as issued by the {@link #registerComponentId(AbstractComponentManager)} method.
   * This counter is only incremented.
   */
  private volatile long       m_componentCounter;

  /**
   * The OSGi service registration for the ScrService provided by this instance.
   */
  ...
  protected ComponentRegistry(BundleContext context) {
    ...
    m_registration = context.registerService(new String[] { ScrService.class.getName(), }, this, props);
  }
  ...
  /**
   * Assigns a unique ID to the component, internally registers the component under that ID and returns the assigned
   * component ID.
   * 
   * @param componentManager
   *          The {@link AbstractComponentManager} for which to assign a component ID and which is to be internally
   *          registered
   * 
  ...
  /**
   * Unregisters the component with the given component ID from the internal registry. After unregistration, the
   * component ID should be considered invalid.
   * 
   * @param componentId
  ...
  /**
   * Checks whether the component name is "globally" unique or not. If it is unique, it is reserved until the actual
   * component is registered with {@link #registerComponent(String, AbstractComponentManager)} or until it is unreserved
   * by calling {@link #unregisterComponent(String)}. If a component with the same name has already been reserved or
   * registered a ComponentException is thrown with a descriptive message.
   * 
   * @param name
   *          the component name to check and reserve
  ...
  /**
   * Registers the given component under the given name. If the name has not already been reserved calling
   * {@link #checkComponentName(String)} this method throws a {@link ComponentException}.
   * 
   * @param name
   ...
   * 
   * @throws ComponentException
   *           if the name has not been reserved through {@link #checkComponentName(String)} yet.
   */
  ...
  /**
   * Returns the component registered under the given name or <code>null</code> if no component is registered yet.
   */
  ...
  /**
   * Returns an array of all values currently stored in the component holders map. The entries in the array are either
   * String types for component name reservations or {@link ComponentHolder} instances for actual holders of components.
   */
  ...
  /**
   * Removes the component registered under that name. If no component is yet registered but the name is reserved, it is
   * unreserved.
   * <p>
  ...
  /**
   * Factory method to issue {@link ComponentHolder} instances to manage components described by the given component
   * <code>metadata</code>.
   */
  ...
  /**
   * Returns <code>true</code> if the <code>bundle</code> is to be considered active from the perspective of declarative
   * services.
   * <p>
   * As of R4.1 a bundle may have lazy activation policy which means a bundle remains in the STARTING state until a
   * class is loaded from that bundle (unless that class is declared to not cause the bundle to start). And thus for DS
   * 1.1 this means components are to be loaded for lazily started bundles being in the STARTING state (after the
   * LAZY_ACTIVATION event) has been sent. Hence DS must consider a bundle active when it is really active and when it
   * is a lazily activated bundle in the STARTING state.
   * 
   * @param bundle
   *          The bundle check
   * @return <code>true</code> if <code>bundle</code> is not <code>null</code> and the bundle is either active or has
   *         lazy activation policy and is in the starting state.
   * 
